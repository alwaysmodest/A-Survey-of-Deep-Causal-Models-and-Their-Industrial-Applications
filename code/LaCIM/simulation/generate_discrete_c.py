# -*- coding: utf-8 -*-
"""
Created on Mon Sep  7 11:05:54 2020
...
def generate_c(n_per_clus, dim_c:int, K_mu=None, seed=None,dtype=np.float32):
    ...
    c = np.array(to_one_hot(label,5)).squeeze().astype(dtype)
    #c = np.matmul(label_one_hot,K_mu)
    return c, label,K_mu
...
def generate_zs(c, K_mu,label, dim_z,dim_s,seed=None):
    ...
    mu_z = K_mu
    log_sigma_z = np.matmul(label_c, Az_sigma) 
    mu_s = K_mu
    log_sigma_s = np.matmul(label_c, As_sigma) 
...
def save_data(n_per_clus, dim_c,dim_z,dim_s,dim_x,dim_y,n_layers,activation,slope=0.5,K_mu=None,dtype=np.float32,seed=None):
    ...
    c, label,K_mu = generate_c(n_per_clus, dim_s, K_mu,seed,dtype)
    z,s,_,_,_,_ = generate_zs(c,K_mu,label,dim_z,dim_s,seed)
    ...
    np.savez_compressed(path, s=s, z=z, x=X,y=y,K_mu=K_mu,c=c,label=label)
    print(' ... done')
    return path
...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.x[index], self.y[index], self.s[index], self.z[index], self.c[index] 